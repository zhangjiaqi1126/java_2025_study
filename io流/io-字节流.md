...
- 获取功能
  ...
  ```
   File f = new File("/Users/zhangjiaqi62/IdeaProjects/java_study/src/test/java/java1.txt");
          System.out.println(f.canRead());//是否可读 true
          System.out.println(f.canWrite());//是否可写 true
          System.out.println(f.canExecute());//是否可执行 false
          System.out.println(f.lengtfalse)// 0
  ```
...
- **读写 时定义 `int`  类型变量 len 的核心原因是：**
  1. **避免 `byte` 类型的有符号性问题（混淆 `255` 和 `-1`）。**
  2. **支持更大的返回值范围（如批量读取时字节数可能超过 `byte` 的容量）。**
  3. **明确流结束标志（用 `-1` 表示结束）。**
...
- 示例代码
  ...
  public class write {
      public static void main(String[] args) throws IOException {
          OutputStream outputStream = new FileOutputStream("/Users/zhangjiaqi62/IdeaProjects/java_study/src/main/resources/ceshi.txt");
          String[] s = new String[]{"张家启测试好厉害", "adasdadasd"};
          for (String s1:s){
